...
if (isset($_POST['submit'])) {
  ...
  if (isset($_GET['edit'])) {
    $id = $_GET['edit'];
    $query = "UPDATE penerbit SET nama='$nama', deskripsi='$deskripsi', tanggal_perubahan='$tanggal_perubahan' WHERE id='$id'";
  }
  // jika tidak ada data yang di kirim di url maka data di tambah
  else {
    $query = "INSERT INTO penerbit(id, nama, deskripsi, tanggal_perubahan) VALUES (null, '$nama', '$deskripsi', '$tanggal_perubahan')";
  }
...
if (isset($_GET['edit'])) {
  ...
  $result = mysqli_query($conn, "SELECT * FROM penerbit WHERE id='$id'");
...
<head>
  ...
  <title><?= $title ?> Data Penerbit | CRUD Data Buku</title>
  <!-- Required meta tags -->
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  <!-- bootstrap template -->
    ...
    <div class="card shadow mt-3">
      <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
          <label class="h6"><?= $title ?> Data Penerbit</label>
          <a href="./penerbit.php" class="btn btn-sm btn-secondary">Kembali</a>
        </div>
      </div>
      <div class="card-body">
        <form method="POST">
          <div class="form-group">
            ...
            <input type="text" class="form-control" name="nama" id="nama" value="<?= $nama ?>" placeholder="Nama Penerbit" required>
          </div>
          <div class="form-group">
         ...
         <!-- <div class="form-group">
            ...
            <input type="datetime" class="form-control" name="tanggal_perubahan" id="tanggal_perubahan" value="<?= $tanggal_perubahan ?>" placeholder="Tanggal Perubahan" required>
          </div> -->
          <div class="form-group">
            <label for="tanggal_perubahan">Tanggal Perubahan</label>
            <textarea class="form-control" name="tanggal_perubahan" id="tanggal_perubahan" rows="3"><?= $tanggal_perubahan ?></textarea>
          </div> 
